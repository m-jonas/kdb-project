...
class MockQConnection:
    def __init__(self):
        # Create a large initial DataFrame to simulate existing data
        # ohlc: time (timespan), sym (symbol), open, high, low, close, volume, vwap
    ...
    def __call__(self, query, *args):
        ...
        if "select from ohlc where time >" in str(query):
            # Delta fetch
            # Extract time from args or query
            # Assuming usage: q("0!select from ohlc where time > x", last_time)
            if args:
                last_time = args[0]
                # Filter data
    ...
    def add_data(self, num_rows=1):
        # Add new rows to simulate real-time data
...
def run_benchmark_original():
    ...
    for i in range(5):
        ...
        # 2. Fetch Data (Original: Full Scan)
        start_fetch = time.time()
        res = q("0!ohlc")
        df = res.pd()
        end_fetch = time.time()

        print(f"Iter {i+1}: Fetched {len(df)} rows in {end_fetch - start_fetch:.4f}s")

    end_total = time.time()
    print(f"Total Time: {end_total - start_total:.4f}s")
    print(f"Total Rows Fetched: {q.rows_fetched}")
    return end_total - start_total, q.rows_fetched

if __name__ == "__main__":
    run_benchmark_original()
